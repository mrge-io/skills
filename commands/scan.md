...
3. **If no issue ID was provided**: Call `get_scan` with the owner and repo. Follow `hasMore`, increasing `offset` by the returned `limit`, until all requested results are loaded. Use `list_scans` only if the repository cannot be identified.
